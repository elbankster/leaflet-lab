...
const XML_URL = 'https://wateroffice.ec.gc.ca/services/current_conditions/xml/inline?lang=en';

// Function to download and update the file in localStorage
async function updateFileIfNeeded() {
    const localStorageKey = 'current_conditions';
    ...
    try {
        ...
        // Check if the file is older than 15 minutes (900000 ms)
        if (lastUpdated && now - parseInt(lastUpdated, 10) < 900000) {
            console.log('File is up-to-date, no need to download.');
            return;
        }

        console.log('File is older than 15 minutes or does not exist, updating...');
        ...
        // Save the file content and timestamp in localStorage
        localStorage.setItem(localStorageKey, fileContent);
        localStorage.setItem(lastUpdatedKey, now.toString());

        console.log('File downloaded and updated successfully.');
    } catch (error) {
...
async function getGaugeDetails(gaugeId) {
    ...
    try {
        // Fetch the XML file
        const response = await fetch('webservice/current_conditions.kml');
